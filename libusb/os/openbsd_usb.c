...
_cache_active_config_descriptor(struct libusb_device *dev)
{
	...
	usbi_dbg("fd %d, addr %u", fd, dev->device_address);
	...
	usbi_dbg("active bLength %u", udc.udc_desc.bLength);

	len = (unsigned int)UGETW(udc.udc_desc.wTotalLength);
	buf = malloc(len);
	if (buf == NULL)
		return (LIBUSB_ERROR_NO_MEM);
	...
	usbi_dbg("index %d, len %u", udf.udf_config_index, len);
...
_sync_control_transfer(struct usbi_transfer *itransfer)
{
	...
	usbi_dbg("type %x request %x value %x index %u length %u timeout %u",
