 ...
 * per \ref libusb_context.
 *
 * It is safe to call libusb_hotplug_deregister_callback() on an already
 * deregistered callback.
 *
 ...
 * Since version 1.0.16, \ref LIBUSB_API_VERSION >= 0x01000102
 *
 * Hotplug callback registration flags */
typedef enum {
	/** Call the callback for all matching currently attached devices. */
	LIBUSB_HOTPLUG_ENUMERATE = (1 << 0)
} libusb_hotplug_registration_flag;
 ...
 *
 * This callback is called when a matching event has happened on a matching
 * device. It is only called when handling events, or from
 * libusb_hotplug_register_callback() when \ref LIBUSB_HOTPLUG_ENUMERATE
 * is provided in flags. It is recommended that the callback do minimal
 ...
 * \param user_data user data pointer provided when this callback was registered
 * \returns an integer indicating whether this callback is finished processing
 * events (0 keeps the callback active, a non-zero value will cause this
