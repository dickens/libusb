...
static int semaphore_init(semaphore_t *semaphore)
{
	return sem_init(semaphore, 0, 0);
}

static void semaphore_give(semaphore_t *semaphore)
{
	(void)sem_post(semaphore);
}

static void semaphore_take(semaphore_t *semaphore)
{
	(void)sem_wait(semaphore);
}

static void semaphore_destroy(semaphore_t *semaphore)
{
	(void)sem_destroy(semaphore);
}

static int thread_create(thread_t *thread,
...
static int thread_create(thread_t *thread,
	thread_return_t (THREAD_CALL *thread_entry)(void *arg), void *arg)
{
#if defined(__CYGWIN__)
...
#elif defined(PLATFORM_WINDOWS)
static thread_return_t THREAD_CALL poll_thread_main(void *arg)
#endif
{
	(void)arg;
